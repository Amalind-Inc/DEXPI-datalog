...
import { fuzzyFind } from "./pi-natives.ts";
import { getProjectDir } from "./pi-utils.ts";
...
function buildSlashCommandCompletions(
  ...
  return commands
    .flatMap((cmd) => {
      ...
      const descScore =
        lowerDesc && fuzzyMatch(lowerPrefix, lowerDesc)
          ? fuzzyScore(lowerPrefix, lowerDesc) * 0.5
          : 0;
 ...
 */
export function midPromptSkillTokenMatches(
  lowerToken: string,
  name: string,
  description?: string,
): boolean {
...
export class CombinedAutocompleteProvider implements AutocompleteProvider {
  ...
  async getSuggestions(
    ...
    const hasPromptTextBeforeLeadingSlash =
      leadingSlashStart !== null &&
      hasPromptTextBeforeSlash(lines, cursorLine, textBeforeCursor, leadingSlashStart);
    ...
    if (slashStart !== null) {
      ...
      if (spaceIndex === -1) {
        ...
        if (
          ...
          slashStart === leadingSlashStart &&
          !commandText.slice(1).includes("/")
        ) {
          return null;
        }
        ...
        if (
          command &&
          "allowArgs" in command &&
          command.allowArgs === false &&
          !/\S/.test(argumentText)
        ) {
          return null;
        }
  ...
  #expandHomePath(filePath: string): string {
    if (filePath.startsWith("~/")) {
      ...
      return filePath.endsWith("/") && !expandedPath.endsWith("/")
        ? `${expandedPath}/`
        : expandedPath;
    } else if (filePath === "~") {
      return os.homedir();
  ...
  async #getFuzzyFileSuggestions(
    ...
    try {
      ...
      const suggestions: AutocompleteItem[] = [];
      for (const { path: entryPath } of topEntries) {
