import { popLoopPhase, pushLoopPhase } from "../pi-utils.ts";
import { fuzzyFilter } from "../fuzzy.ts";
import { getKeybindings } from "../keybindings.ts";
import { extractPrintableText } from "../keys.ts";
import { type MouseRoutable, routeSelectListMouse, type SgrMouseEvent } from "../mouse.ts";
...
export class SelectList implements Component, MouseRoutable {
  ...
  render(width: number): readonly string[] {
    ...
    for (let i = startIndex; i < endIndex && rows.length < visualBudget; i++) {
      ...
      if (!item) continue;
      const hovered =
        this.#theme.hovered !== undefined && i === this.#hoveredIndex && i !== this.#selectedIndex;
  ...
  #renderStatusLine(width: number): string {
    ...
    return this.#theme.scrollInfo(
      truncateToWidth(statusText, Math.max(1, width - 2), Ellipsis.Omit),
    );
  }
