...
No raw XML, graph export, README guidance, rule pack, oracle, or project reasoning code is available in this locked arm.

## Question

Piping attachment integrity: find every Nozzle on this drawing that is not attached to the piping network, i.e. it never appears as the target of a reference edge whose attr_name is sourceItem, targetItem, sourceNode, or targetNode. Check every nozzle exhaustively against every reference edge. If any such object exists, answer violation_found and list every one of them (and only them) as witnesses; if none exists, answer no_violation with no witnesses.
...
Write `/workspace/structured_answer.json` containing exactly one JSON object:

```json
{
  "verdict": one of ["violation_found", "no_violation", "unanswerable"],
  "witness_ids": a list of object IDs from the drawing that are the evidence
    for your verdict (empty if none apply),
  "posture": one of ["unspecified", "source_grounded", "general_knowledge", "source_data_unavailable", "out_of_scope", "needs_clarification"],
...
Also leave `/workspace/analysis.dl` containing the exact Datalog
